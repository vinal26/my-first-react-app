...
const AllPatientConnect = (props) => {
  ...
  const getList = async() => {
    try {
      const response = await getLifestyleList(lifeStyleCategoryList.connect, props.userid, props?.selectedTemplate);
      if(response) {
    ...
    <>
      <div className="row mt-4">
        <div className="col-md-6">
          <Affirmations isDisable={props.isDisable} selectedTemplate={props?.selectedTemplate} userid={props.userid} isMandatory={isMandatory} list={affirmationList} categoryName={lifeStyleCategoryList.connect} onChangeCheckBox={onChangeCheckBox}/>
        </div>

        <div className="col-md-6">
          <Checklist isDisable={props.isDisable} selectedTemplate={props?.selectedTemplate} userid={props.userid} isMandatory={isMandatory} categoryName={lifeStyleCategoryList.connect} list={checkList} onChangeCheckBox={onChangeCheckBox}/>
        </div>
      </div>
    </>
  );
...
export default AllPatientConnect;
