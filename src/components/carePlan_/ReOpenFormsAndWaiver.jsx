...
import { getFormSelfResponses } from "../../services/PatientOverviewServices";
...
const ReOpenFormsAndWaiver = () => {
    ...
    let answerid = searchParams.get("id")
    ...
    const saveProgram = async () => {
        try {
            const response = await formListSumbitService(formid, {
                ...
                "questions": Object.entries(allForm).map(dt => {
                    return dt[1];
                })
            });
            setLoader(false);
    ...
    const getFormList = async () => {
        try {
            ...
            let resp = await getFormSelfResponses(userid, formid)
            if (resp?.status == 200) {
                setFormAns(resp.data.data[0])
                // setFData(resp.data.data[0]);
                // setFormName(resp.data.data[0]?.formName);
                // setDescription(resp.data.data[0]?.description)

                console.log(resp.data.data, 'Forms & Answers');
            }
        } catch (e) {
            // showToastError("Unable to Fetch User Symptoms")
            setLoader(false);
            console.log(e, "Forms & Errors")
        }
    }

    const prefilled = () => {
        let obj = {};
        FormAns.questions.map((element, index) => {
            obj[+index] = { ...element }
        })
        setAllForm(obj)
    }
                        ...
                        <div className="row add-recipe">
                            ...
                            <div className="col-md-12">
                                {
                                    ...
                                    FormAns?.questions?.map((item, index) => {
                                        if (item.type == 'info') {
                                            return (
                                                <div key={index}>
                            ...
                            <div className="col-md-12 mt-5">
                                ...
                                <div className="d-flex justify-content-between">
                                    ...
                                    <Button isLoading={btnLoad} type="submit" id="jhsv" text={'Re-Submit'} style={btnLoad ? { pointerEvents: 'none' } : {}} className="description_btnsave mx-0" />
                                </div>
                            </div>
                        </div>
...
export default ReOpenFormsAndWaiver;
